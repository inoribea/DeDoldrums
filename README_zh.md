...
| 不同之处 | 为什么重要 |
|:---|:---|
| **强制对抗闸门** | 综合合成后，管线会物理性阻断——直到至少一条关键发现通过结构化挑战（逻辑漏洞、隐藏假设、缺失证据、替代解释）。不是让模型"自己检查一下"，是硬性阻断。未通过的发现标记为需修正，随挑战结果一起进入同行评审。 |
...
底层：5.5 阶段 STORM 管线、动态视角发现、扇出并行子代理、跨会话持久化的 L0-L4 分层记忆。
...
Stage 3.5 — ⚔️ 对抗验证闸门  （阻断直至 ≥1 条挑战执行；标记发现随结果进入同行评审）
Stage 4   — 同行评审        （置信度评分、偏见检测、遗漏视角）
```

> Stage 3.5 不是让模型"检查一下自己的作业"。它是硬性闸门——没有 `challenge` 工具的真实调用，管线不会推进。
...
| **STORM 社区衍生版** | 矛盾映射、同行评审机制 | 与对抗闸门的紧密集成——闸门标记的发现直接流入同行评审 |

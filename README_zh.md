...
下面的编号是本项目改造中落地的具体保障机制，不是论文术语。

| 编号 | 保障机制 | 实际做什么 |
|:---|:---|:---|
| P1 | 有界证据交接 | 每个扇出子代理最多只回传 10 条紧凑的 `(claim, source_url, confidence)` 记录，不把原始研究长文灌回主上下文。 |
| P2 | 文档级审计 | 闸门通过前，检查整份研究的来源可定位性、反证、盲点诚实度、标签正确性、透镜覆盖度和 claim-source 支持度。 |
| P3 | 强制盲点声明 | 每份综合结果都必须有具体的 `## Blind Spots` 节，说明搜过什么、没试什么，以及这些缺口会怎样影响结论。 |
| P4 | 人工检查点 | 默认在闸门后、最终简报前暂停，让人查看矛盾地图、综合结果和审计缺口。 |
| P5 | 结构化 findings 台账 | 每条 claim 统一记录断言、可定位来源 URL、置信度和透镜；没有来源的断言自动降级为「猜测」。 |
| P6 | 压缩前先持久化 | 上下文截断前先把阶段产物写入会话归档，研究中断后可以恢复。 |
| P7 | 文档与布局对齐 | README 架构图保持与仓库真实的根目录布局一致。 |
| P8 | 显式闸门凭证 | 最终简报必须有通过的文档审计，或一条具体记录的降级理由；仅调用过 challenge 不能打开闸门。 |

以下 2026 年 arXiv 预印本指出的失效模式和设计目标，与这些保障机制趋同。它们作为独立佐证引用，而非实现来源——DeDoldrums 没有继承或实现这些工作的代码、框架或评估管线。

| 预印本 | 对齐点 |
|:---|:---|
| [**From Fluent to Verifiable**](https://arxiv.org/abs/2602.13855)（Rasheed et al., 2026 预印本） | 同样将来源覆盖度、来源可靠性与矛盾透明度作为深度研究 agent 的一等审计目标。 |
| [**Cited but Not Verified**](https://arxiv.org/abs/2605.06635)（Onweller et al., 2026 预印本） | 同样检查引用来源是否可达、主题相关、且确实支持断言；表层引文质量不等于事实可靠性。 |
| [**AutoResearch**](https://arxiv.org/abs/2607.02520)（Kumar et al., 2026 预印本） | 同样将引文验证与 claim-support 审计作为多 agent 研究工作流的运行期过滤信号。 |
